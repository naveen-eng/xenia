...
export default class Leaves extends Component {
  ...
  save_task=async()=>{
    ...
    xhr.open("POST", "http://erp.xeamventures.com/api/v1/save-task");
    ...
    xhr.send(data);
     ...
     createTask=async()=>{
        ...
        xhr.open("GET", "http://erp.xeamventures.com/api/v1/create-task");
        xhr.setRequestHeader("Authorization", "Bearer " + permissions_sec);
        xhr.send(data);
     }
    ...
    render (){
            const context=this;
            const {task,task_title,task_projects_id,employees_id,}= this.state;
		...
		return(
            ...
            <ScrollView>
            <View style={{flexDirection:'column',bottom:hp(1),left:'3%',}}>
              
            
            <Dropdown
              containerStyle={{width:'88%',left:'2%'}}
              inputContainerStyle={{ borderBottomWidth: 1,borderBottomColor:"rgb(19,111,232)" }}
              data={this.state.task_projects}
              valueExtractor={({id})=> id}
              labelExtractor={({name})=> name}
              label='Task project'
              onChangeText={task_projects_id => this.setState({ task_projects_id })}
            />
            </View>
            {(this.state.loading) ?
            <View style={{
            ...
            <TextInput
                ...
                <View style={{flexDirection:'row',marginLeft:'5%',marginRight:'5%',top:hp(2)}}>
                  {this.state.priority!=='H1' ?
                    <TouchableOpacity style={{backgroundColor:'#fffdd0',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H1"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H1</Text>
                    </TouchableOpacity>:null}
                    {this.state.priority=='H1' ?
                    <TouchableOpacity style={{backgroundColor:'#adadad',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H1"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H1</Text>
                    </TouchableOpacity>:null}

                    {this.state.priority!=='H2' ?
                    <TouchableOpacity style={{backgroundColor:'#ffff00',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H2"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H2</Text>
                    </TouchableOpacity>:null}
                    {this.state.priority=='H2' ?
                    <TouchableOpacity style={{backgroundColor:'#adadad',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H2"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H2</Text>
                    </TouchableOpacity>:null}

                    {this.state.priority!=='H3' ?
                    <TouchableOpacity style={{backgroundColor:'#00ffff',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H3"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H3</Text>
                    </TouchableOpacity>:null}
                    {this.state.priority=='H3' ?
                    <TouchableOpacity style={{backgroundColor:'#adadad',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H3"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H3</Text>
                    </TouchableOpacity>:null}

                    {this.state.priority!=='H4' ?
                    <TouchableOpacity style={{backgroundColor:'#ffa500',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H4"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H4</Text>
                    </TouchableOpacity>:null}
                    {this.state.priority=='H4' ?
                    <TouchableOpacity style={{backgroundColor:'#adadad',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H4"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H4</Text>
                    </TouchableOpacity>:null}

                    {this.state.priority!=='H5' ?
                    <TouchableOpacity style={{backgroundColor:'#ff0000',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H5"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H5</Text>
                    </TouchableOpacity>:null}
                    {this.state.priority=='H5' ?
                    <TouchableOpacity style={{backgroundColor:'#adadad',width:'20%',height:'200%',}} onPress={()=>this.setState({priority:"H5"})}>
                        <Text style={{color:'black',textAlign:'center',top:'20%'}}>H5</Text>
                    </TouchableOpacity>:null}

                </View>
                <View style={styles.pagecomponent_nine}/>
                <View style={{left:'0%',bottom:hp(7.5),alignItems:'center'}}>
              <Text style={{top:'0%',backgroundColor:'white',color:'rgb(19,111,232)',}}>   Assigner Person Details   </Text>
              </View>
             
             <View style={{flexDirection:'row',bottom:hp(8),left:'4%',alignItems:'center'}}>
            ...
            />
            <Dropdown
              containerStyle={{width:'37%',left:'50%'}}
              inputContainerStyle={{ borderBottomWidth: 1,borderBottomColor:"rgb(19,111,232)" }}
            ...
            <DatePicker
              ...
              <View style={{flexDirection:'row',alignItems:'center',justifyContent:'center',bottom:hp(0),margin:0}}>

         <TouchableOpacity style={{right:'0%',top:'0%'}} onPress={context.Reminder}>
            {this.state.reminder=== 'off' ?
            <Image source={require('../src/Image/unchecked.png')} style={{ width: wp(7), height: hp(4), marginLeft:wp(0),top:0 }}/>
            : null}
            { this.state.reminder=== 'on' ?
        <Image source={require('../src/Image/checked.png')} style={{ width: wp(7), height:hp(4), marginLeft: wp(0),top:0 }}/>
           :null}
        ...
        <Image source={require('../src/Image/checked.png')} style={{ width: wp(7), height:hp(4), marginLeft: wp(0),top:0 }}/>
          ...
          <View style={{bottom:hp(0),left:wp(5)}}> 
          { this.state.reminder == 'on' ?
          <Dropdown
          ...
          <Text style={{color:'white',top:'15%',left:'30%'}}>
           Choose file (Optional)
              ...
              <View style={styles.TaskDescription}>
                   ...
                   <TextInput
                   style={{ height: 110, borderColor: 'transparent', borderWidth: 1, bottom:'10%',width:'100%',fontSize:15,keyboardAppearance:'light' }}
                 ...
                 />
                   ...
                   <View style={{top:hp(12),marginBottom:hp('45%')}}>
                       <TouchableOpacity style={{alignItems:'center',}} onPress={()=>this.save_task()}>
